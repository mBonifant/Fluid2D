 ...
 * 
 * Much of this code is taken or else adapted from LatticeBoltzmannDemo.java By
 * Daniel V. Schroeder.
 * (http://physics.weber.edu/schroeder/fluids/LatticeBoltzmannDemo.java.txt)
 * particularly the way the lattice is implemented and the
 * streaming/collision/bouncing
 * 
 * Schroeder's GUI was ignored, and a listener was added for changes in the
 * lattice at each step/iteration, witch changes in the gui, the source of the
 * omega term was shifted to the Liquid object/class. And sources and sinks were
 * additionally added. A TODO would be to adjust the source/sink features to
 * change the in/out flow of fluid of the sources/sinks collectively or even
 * individually.
 * 
 * @author bonifantmc
 * 
...
public class Lattice {
	...
	/**
	 * @return the speed squared of the default in flow
	 */
	...
	/**
	 * get the color of a given pixel in the lattice
	 * 
	 * @param cs
	 *            which Stat to base the retrieved Color on
	 * @param factor
	 *            multiple the value in the lattice by this, to intensify it
	 * @param i
	 *            the i/x coordinate of the pixel to get
	 * @param j
	 *            the j/y coordinate of the pixel to get
	 * @return the pixel color (which is the hsvtoRGB, the h is the value at the
	 *         given coordinate multiplied by the given facotr)
	 */
	public Color getColor(ColorStats cs, float factor, int i, int j) {
		...
		return Color.getHSBColor(f * factor, 0.7f, 0.9f);
	}

...
public class DrawingTool extends MouseAdapter implements KeyListener {
	...
	/** The editlattice using this adapter */
	private final EditLattice edit;

	/** This tool's editlattice's Boundaries when first displayed */
	private final BoundarySnapshot initial;

	/** Stack of previous Boundaries as this tools editlattice was edited */
	private final Stack<BoundarySnapshot> undoStack;

	/**
	 * Current set of Boundaries for this tool's editlattice, to add to stack
	 */
	 ...
	 * <p>
	 * right-clicking: display a pop-up menu of features to switch between
	 * <p>
	 * left-click: Begin drawing an Boundary
	 * <p>
	 * double-left-click: delete a Boundary
	 */
	@Override
	public void mousePressed(MouseEvent e) {
		...
		if (e.getClickCount() == 1) {
			...
			if ((a = Boundary.nearBoundary(pt, list)) != null) {
				...
				// translate boundary when not drawing a boundary
				if (
	...
	/**
	 * 
	 * Handle key commands the letters eriowfc for switching between, ellipses
	 * and rectangles and inflow, outflow, wall, fluid, (also for clearing) and
	 * ctrl-z for undo.
	 * 
	...
	/**
	 * if a new boundary add it to the list and start drawing
	 */
	void startDrawingBoundary() {
		setTempBoundary(new Boundary(getDrawingFeature(), getAnchor().x, getAnchor().y, 0f, 0f));
